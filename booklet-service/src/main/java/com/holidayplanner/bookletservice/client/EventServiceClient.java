...
public class EventServiceClient {
  ...
  public List<EventTermDto> getTermsForEvent(UUID eventId) {
    try {
      List<EventTermDto> result =
          restClient
              .get()
              .uri(eventServiceUrl + "/api/events/" + eventId + "/terms")
              .header("X-Service-Secret", serviceSecret)
              .retrieve()
              .body(new ParameterizedTypeReference<List<EventTermDto>>() {});
      return result != null ? result : List.of();
    } catch (RestClientException e) {
      throw new UpstreamServiceException("Could not fetch event terms from event-service", e);
    }
  }
}
