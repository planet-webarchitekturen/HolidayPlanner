 ...
 *
 * Runs on a fixed schedule. Each event is only marked processed after the broker
 * acknowledges it ({@code .get()} blocks on the send result), giving at-least-once
 * delivery. On the first failure we stop the batch to preserve per-aggregate
 * ordering; the remaining rows are retried on the next tick.
 */
@Slf4j
@Component
...
public class OutboxRelay {
    ...
    public void publishPendingEvents() {
        ...
        // Managed entities: processed flags flush on transaction commit.
    }
}
