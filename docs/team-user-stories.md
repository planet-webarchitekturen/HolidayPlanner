...
| Team member | Stories |
|---|---|
| Dev A | 1 (Create Booking integrity) + 7 (Family member veto) — both touch booking+identity |
| Dev B | 3 (Refunds) + 2 (Cancellation rules) — payment/booking pair |
| Dev C | 5 (Capacity consumer) + 4 (Scheduler tests) — Kafka-focused |
| Dev D | 6 (PDF notification) + 10 (Email delivery) — notification/booklet pair |
| Dev E | 9 (Auth/JWT) + 8 (Delete-org saga decision) |
